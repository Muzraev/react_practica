...
function DataExporter({ technologies }) {
  ...
  const exportData = () => {
    try {
      const exportData = {
        ...
        technologies: includeUserData 
          ? technologies.map(tech => ({
              ...
              userResources: tech.resources || []
            }))
          : technologies.map(({ notes, status, deadline, resources, ...tech }) => tech)
      };
      ...
      setExportStatus(`✅ Успешно экспортировано ${technologies.length} технологий`);
